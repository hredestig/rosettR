...
Image analysis can be done in parallel to speed up computation considerably. The `rosettR` package uses [plyr](https://cran.r-project.org/web/packages/plyr/index.html) which support using multiple CPUs on a single host via the [doParallel](https://cran.r-project.org/web/packages/doParallel/index.html) package. Simple register a parallel backend by for using e.g. 4 CPUs by
